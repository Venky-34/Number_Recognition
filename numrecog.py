# # importing the necessary libraries
import os
import cv2
import numpy as np
import matplotlib.pyplot as plt
import tensorflow as tf

#pre-processing
mnist = tf.keras.datasets.mnist                         #loading the mnist data
(x_train, y_train),(x_test,y_test) = mnist.load_data()  #catogerizing into traing and testing data

x_train = tf.keras.utils.normalize(x_train, axis=1)     #normalizing the data
x_test = tf.keras.utils.normalize(x_test, axis=1)       #normalizing the data

#creating the model to train the data

model = tf.keras.models.Sequential()
model.add(tf.keras.layers.Flatten(input_shape=(28,28)))
model.add(tf.keras.layers.Dense(128,activation='relu'))
model.add(tf.keras.layers.Dense(128,activation='relu'))
model.add(tf.keras.layers.Dense(10,activation='softmax'))
model.summary()

#Compiling the model
model.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])

# fit the model
model.fit(x_train, y_train, epochs=10,)


#Saving the model that we have created for our future use.
model.save('Number_Recognition.model')

# Using the model that we have created for printing the loss and accuracy
model = tf.keras.models.load_model('number_recognition.model')
loss, accuracy = model.evaluate(x_test,y_test)
print("loss is: ", loss)
print("accuracy is: ", accuracy)




#Predicting the handwritten images
image_number = 1
while os.path.isfile(f"digits/digit{image_number}.png"):
    try:
        img = cv2.imread(f"digits/digit{image_number}.png")[:,:,0]
        img = np.invert(np.array([img]))
        prediction = model.predict(img)
        print(f"the number is probably:  {np.argmax(prediction)}")
        plt.imshow(img[0],cmap=plt.cm.binary)
        plt.show()
    except:
        print("Error..!")
    finally:
        image_number +=1